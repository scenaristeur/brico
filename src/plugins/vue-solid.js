...
const plugin = {
  install(Vue, opts = {}) {
    ...
    Vue.prototype.$getFolder = async function(url){
      let session = sc.getDefaultSession()
      let dataset
      if (session.info.isLoggedIn == true){
        dataset = await getSolidDataset( url, { fetch: sc.fetch });
      }else{
        dataset = await getSolidDataset( url);
      }
      let graphs_default = dataset.graphs.default
      let folder = Object.values(graphs_default)
      return folder
    }
    ...
    Vue.prototype.$getPodInfos = async function(pod){
      try{
        ...
        let dataset
        if (session.info.isLoggedIn == true){
          dataset = await getSolidDataset( pod.webId, { fetch: sc.fetch });
        }else{
          dataset = await getSolidDataset( pod.webId);
        }
    ...
    Vue.prototype.$label = async function (url) {
      return await lastPart(url)
    }
